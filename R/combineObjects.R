  ...
  if (any(lapply(dicoCodings, compareDicos) == FALSE)){
    ...
    for (i in 1:length(wearableCamImagesList)) {
        ...
        codesBinaryVariables <- rbind(codesBinaryVariables,
                                      wearableCamImagesList[[i]]@codesBinaryVariables)# nolint
        participantID <- c(participantID,
                           wearableCamImagesList[[i]]@participantID)
    }
    wearableCamImagesObject <- new("wearableCamImages", dicoCoding = dicoCoding,
                                   imagePath = imagePath,
