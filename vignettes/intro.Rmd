...
* Columns of boolean, indicating if a given code was given to a given picture.
...
In the case of the CHAI project, we had many coding results files from XnView MP and some of them had annotations spread over several columns. Because of this we added the `robust_reading` option which is FALSE by default. When TRUE each results file is read twice, once for finding image_time and image_path, once for finding annotation by considering each line as a single variable. This way we didn't need to care too much about the number of columns of annotations, even if in theory it should have been one column only.
